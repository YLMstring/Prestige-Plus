...
namespace PrestigePlus
{
    public static class Main
  {
    ...
    public static readonly LogWrapper Logger = LogWrapper.Get("PrestigePlus");
        private static readonly string RootKey = "mod-menu.pp-settings";
       ...
       ModMenu.ModMenu.AddSettings(
        SettingsBuilder.New(RootKey, Helpers.CreateString("title", "Prestige Plus"))
          ...
          .AddToggle(
            Toggle.New(GetKey("speardance"), defaultValue: true, Helpers.CreateString("toggle-desc104", "Spear Dancing Style"))
              .ShowVisualConnection())
          .AddToggle(
            Toggle.New(GetKey("deadlyag"), defaultValue: true, Helpers.CreateString("toggle-desc105", "Deadly Agility"))
              .ShowVisualConnection())
          .AddToggle(
          ...
          .AddToggle(
            Toggle.New(GetKey("enhancemore"), defaultValue: true, Helpers.CreateString("toggle-desc111", "More Mythic Features"))
              .ShowVisualConnection())
          .AddToggle(
            Toggle.New(GetKey("lightcheat"), defaultValue: true, Helpers.CreateString("toggle-desc112", "Like the Sun"))
              .ShowVisualConnection())
          .AddToggle(
                ...
                try
        {
          ...
          Initialized = true;
                    ...
                    if (ModMenu.ModMenu.GetSettingValue<bool>(GetKey("spire"))) { SpireDefender.Configure(); SpireDefender.CreateReachSpellstrike(); SpireDefender.CreateCloseRange(); SpireDefender.CreateDisruptive(); BodyGuard.MagusPetConfigure(); }
                    ...
                    if (ModMenu.ModMenu.GetSettingValue<bool>(GetKey("superop"))) { HermeanPotential.Configure(); DebilitatingPain.Configure(); DebilitatingPainMass.Configure(); GlimpseAkashic.Configure(); AkashicForm.Configure(); PhantomLimb.Configure(); Supernaturalist.Configure(); SwordLord.CreateEsotericDragons(); }
