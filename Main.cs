       ...
       ModMenu.ModMenu.AddSettings(
                ...
                try
        {
          ...
          Initialized = true;
                    ...
                    SpellbookReplace.Select(); SpellbookLevelUp.Select(); ImbueArrow.Configure(); ShadowDancer.CreateProficiencies(); ShadowDancer.ExtraShadowJump(); ShadowDancer.ExtraFeat(); SeizetheOpportunity.Configure(); AerialAssault.ConfigureAbility(); FakeAlignedClass.Configure();
                    ...
                    if (ModMenu.ModMenu.GetSettingValue<bool>(GetKey("spell"))) { LitanyRighteousness.Configure(); ShieldOther.Configure(); }
      ...
      static void LoadPackTOC()
      {
        try
        {
          if (Initialized)
          {
            ...
            Initialized = true; PatchDomain.Patch(); PatchArmorTraining.Patch(); PatchSkaldSharing.Patch();
