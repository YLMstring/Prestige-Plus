...
- Fix Power Attack: now working as descripted: This bonus to damage is halved (–50%) if you are making an attack with an off-hand weapon. Note that Piranha Strike also has the same bug but it isn't fixed because this might give Piranha Strike an edge over Power Attack, which is pretty coool.
